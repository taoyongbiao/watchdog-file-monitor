...
class GitManager:
    ...
    def init_repo(self):
        ...
        try:
            ...
            try:
                ...
                # 设置用户信息（如果尚未设置）
                try:
                    self.repo.config_writer().set_value("user", "name", "FileMonitor").release()
                    self.repo.config_writer().set_value("user", "email", "filemonitor@example.com").release()
    ...
    def get_file_diff(self, file_path, commit_hash=None):
        ...
        try:
            ...
            # 获取文件差异
            diff = self.repo.git.diff(commit.parents[0].hexsha, commit.hexsha, '--', file_path)
            return diff
        except Exception as e:
